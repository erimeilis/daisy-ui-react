...
import type { IModel, IModelEditProps } from '@/types/models';
...
export function ModelEdit<T extends IModel>({
    ...
    const validateField = (key: string, value: unknown): string | null => {
        if (key === 'email') {
            ...
            if (data.type === 'email') {
                if (!value || typeof value !== 'string' || value.trim() === '') return 'Email is required when type is "email"';
        ...
        if (key === 'domain') {
            ...
            if (data.type === 'domain') {
                if (!value || typeof value !== 'string' || value.trim() === '') return 'Domain is required when type is "domain"';
        ...
        if (key === 'type') {
            if (!value || typeof value !== 'string') return 'Type is required';
            if (!['email', 'domain'].includes(value)) return 'Type must be either "email" or "domain"';
        }
        
        if (key === 'picture' && value) {
            if (typeof value !== 'string') return 'Picture URL must be a string';
            try {
                new URL(value);
            } catch {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            Object.keys(cleanedData).forEach(key => {
                if ((cleanedData as Record<string, unknown>)[key] === '') {
                    (cleanedData as Record<string, unknown>)[key] = null;
                }
            });

            // Additional cleaning for allowed-emails: ensure mutually exclusive fields
            if ((cleanedData as Record<string, unknown>).type === 'email') {
                (cleanedData as Record<string, unknown>).domain = null; // Clear domain when type is email
            } else if ((cleanedData as Record<string, unknown>).type === 'domain') {
                (cleanedData as Record<string, unknown>).email = null; // Clear email when type is domain
            }
